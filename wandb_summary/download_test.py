...
class DownloadTests(unittest.TestCase):
    ...
    def test_get_results(self):
        results = wandb_summary.download.get_results(
    ...
    def test_serialize_run(self):
        result = wandb_summary.download.serialize_run(self.__test_run)
        self.assertEqual(result['tags'], '["tag1", "tag2"]')
        self.assertEqual(result['url'], 'test/url')
        self.assertEqual(result['name'], 'test project')
        self.assertEqual(result['state'], 'finished')
    ...
    def test_flatten_dict_and_add(self):
        dict_1 = {'a': 1}
        dict_2 = {'b': 2}
        wandb_summary.download.flatten_dict_and_add(dict_1, dict_2, 'prefix')
        self.assertEquals(dict_2['prefix_a'], 1)

    def test_is_empty_value_primitive(self):
        self.assertFalse(wandb_summary.download.is_empty_value(1))

    def test_is_empty_value_true_str(self):
        self.assertTrue(wandb_summary.download.is_empty_value(
            '{"desc":null,"value":0}'
        ))

    def test_is_empty_value_false_str(self):
        self.assertFalse(wandb_summary.download.is_empty_value('test'))
    ...
    def test_is_empty_value_false_dict(self):
        self.assertFalse(wandb_summary.download.is_empty_value(
            {'desc': 'other', 'value': 0}
        ))
